...
def on_message(ws, message):
    ...
    if not df.empty:
        ...
        column_widths = {'timestamp': 15, 'pair': 8, 'c': 8, 'v': 6, 'ma5': 6, 'ma10': 6, 'cross': 6, 'usd_balance': 12, 'btc_balance': 12, 'total_portfolio_value': 24}
        
        # Print column headers only once or when the DataFrame is reset
        if df.index[-1] == 0:
            headers = ''.join([title.ljust(column_widths[title]) for title in df.columns])
            print(headers)

        # Format and print the last row
        last_row = df.iloc[-1]
        formatted_row = ''.join([str(last_row[col]).ljust(column_widths[col]) for col in df.columns])
        print(formatted_row)
...
def on_close(ws, *args, **kwargs):
    ...
    csv_file_path = "C:/Users/Sam/Desktop/websocket_test.csv"
